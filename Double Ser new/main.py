...
def run(args):
    ...
    for gepoch in range(args.global_epochs):
        ...
        for globalClients in range(NUM_GROUPS):
            ...
            bob = sy.VirtualWorker(hook, id="bob"+str(gepoch*NUM_GROUPS + globalClients))
            alice = sy.VirtualWorker(hook, id="alice"+str(gepoch*NUM_GROUPS + globalClients))
            crypto_provider = sy.VirtualWorker(hook, id="crypto_provider"+ str(gepoch*NUM_GROUPS + globalClients))
            workers[gepoch*NUM_GROUPS + globalClients] = [alice, bob]
            sy.local_worker.clients = workers[gepoch*NUM_GROUPS + globalClients]

            encryption_kwargs[gepoch*NUM_GROUPS + globalClients] = dict(
                workers=workers[gepoch*NUM_GROUPS + globalClients], crypto_provider=crypto_provider, protocol=args.protocol
            )
            kwargs[gepoch*NUM_GROUPS + globalClients] = dict(
                requires_grad=args.requires_grad,
                precision_fractional=args.precision_fractional,
                dtype=args.dtype,
                **encryption_kwargs[gepoch*NUM_GROUPS + globalClients],
            )
            if args.preprocess:
            ...
            for localClients in range(2):
                ...
                if not args.public:
                    model[gepoch*NUM_GROUPS + globalClients].encrypt(**kwargs[gepoch*NUM_GROUPS + globalClients])
                    if args.fp_only:  # Just keep the (Autograd+) Fixed Precision feature
                        model[gepoch*NUM_GROUPS + globalClients].get()

                if args.train:
                    for epoch in range(args.local_epochs):
                        optimizer = optim.SGD(model[gepoch*NUM_GROUPS + globalClients].parameters(), lr=args.lr, momentum=args.momentum)
                        ...
                        # print("start train")
                        train_time = train(args,model[gepoch*NUM_GROUPS + globalClients], federated_train_loaders[client], optimizer, epoch)
                        # print("end train")
                    
                if localClients == 0:

                    model[gepoch*NUM_GROUPS + globalClients].decrypt()
                    # print("local_model 0")
                    # print(model[gepoch*NUM_GROUPS + globalClients].fc1.weight.data)
                    ...
                    print("local_model 1")
                    print(model[gepoch*NUM_GROUPS + globalClients].fc1.weight.data)
                    local_model = copy.deepcopy(model[gepoch*NUM_GROUPS + globalClients])
                    if not args.public:
            ...
            if globalClients ==(NUM_GROUPS-1):
                global_model.fc1.weight.data = (global_model.fc1.weight.data )/ NUM_GROUPS
                global_model.fc2.weight.data = (global_model.fc2.weight.data )/ NUM_GROUPS
                global_model.fc3.weight.data = (global_model.fc3.weight.data )/NUM_GROUPS
