...
def run(args):
    ...
    # alice = DataCentricFLClient(hook, "ws://localhost:5000")
    # bob = DataCentricFLClient(hook, "ws://localhost:5001")
    # crypto_provider = DataCentricFLClient(hook, "ws://localhost:5002")
    ...
    for gepoch in range(args.global_epochs):
        ...
        if not args.public:
            ...
            if args.fp_only:  # Just keep the (Autograd+) Fixed Precision feature
                global_model.get()
                print("sahar: mire to in function ")
        print(global_model.fc1.weight.data)
        start_server(args,workers_a,global_model, kwargs1, local_model, kwargs, gepoch)
